...
def extract_copy_code(html_content):
    """
    Extract value from HTML content by looking for element with id="copy-code".

    Args:
        html_content (str): HTML content to parse

    Returns:
        str or None: The text content of the copy-code element if found, None otherwise
    """
    if not html_content:
        return None
    ...
    try:
        soup = BeautifulSoup(html_content, "html.parser")
        copy_code_element = soup.find("span", id="copy-code")

        if copy_code_element:
            return copy_code_element.get_text(strip=True)
    except Exception as e:
        print(f"Error parsing HTML: {e}")
...
def get_latest_email_html(inbox_id="opentable@agentmail.to", api_key=None):
    """
    Get the HTML content from the latest email in the specified inbox.
    ...
    Returns:
        str or None: The HTML content of the latest email if found, None otherwise
    """
...
def get_copy_code(inbox_id="opentable@agentmail.to", api_key=None):
    """
    Get the copy code from the latest email in the specified inbox.
    ...
    Returns:
        str or None: The copy code if found, None otherwise
    """
    # Get the HTML content from the latest email
    html_content = get_latest_email_html(inbox_id, api_key)
    
    if html_content is None:
        return None
    
    # Extract and return the copy code
    return extract_copy_code(html_content)
